...
func _on_body_entered(body: Node2D) -> void:
	...
	hath just entered unto the berry node's CollisionShape
	...
	if body.name == "player": #determines whether the provided body node is in fact, the aforementioned player node.
		if GameState.b_grown: #conducts a conditional evaluation to determine if the boolean assumes a true value.
			...
			if GameState.inventory["berries"] != GameState.customers[GameState.customer_index][3]:
				body.interact = "berries" #changes a player variable responsible for storing objects it is in contact with
